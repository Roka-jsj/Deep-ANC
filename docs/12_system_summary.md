...
| CPU 클록 | 사양 2.20 GHz / **현재 상한 1.73 GHz** | `scaling_max_freq` |
...
| **전원 모드** | **MODE_30W (ID 2)** | `nvpmodel -q` |

> [!IMPORTANT]
> **전원 모드가 30W다. 이 보드는 MAXN(0)과 MODE_50W(3)를 지원한다.**
> 30W에서 CPU 클록이 **2.20 → 1.73 GHz (79%)** 로 묶인다. 이 문서의 **모든 지연 수치는
> 30W에서 측정**된 것이며, 모드를 올리면 CPU·GPU 양쪽이 함께 빨라진다.
> 전환에는 **재부팅이 필요**하다.
>
> ```bash
...
| 모델 | 파라미터 | 블록 | GPU P50 | GPU P99 | CPU P50 | CPU P99 |
|---|---:|---:|---:|---:|---:|---:|
| `tiny` | 1.16M | 8 | **0.56** | 3.27 | 1.38 | **1.54** |
| `base` | 5.99M | **15** | 1.33 | 6.00 | 6.00 | 6.40 |
| `tiny_wide` | 12.8M | 8 | **1.13** | **4.63** | 9.51 | 10.85 |

**폭은 GPU에 거의 공짜, 깊이는 비싸다.** `tiny_wide`는 `tiny`보다 11배 큰데 GPU에서
2배만 느리고, 파라미터가 2.1배 많은 `base`보다도 **빠르다** — 블록이 8 대 15이기 때문이다.
비용을 내는 것은 순차 커널 런치이지 연산량이 아니다.

**12.8M 모델은 GPU에서만 실시간이 된다** (P99 4.63 ms < 예산 5.33 ms). CPU는 중앙값부터 2배 초과.

TensorRT 엔진 최적화로 P50이 1.32 → 0.56 ms가 됐다(CUDA Graph, pinned 메모리,
spin 동기). 그래프 경로는 비그래프와 **비트 단위로 동일**하다.

### 4.5 실기 ANC (사전학습 tiny + ORT CPU)
...
| 순위 | 병목 | 크기 | 조치 | 필요 자원 |
|---|---|---|---|---|
| 1 | 모델이 실측에서 **증폭**(+1.23 dB) | 결정적 | **파인튜닝** | GPU (진행 중) |
| 2 | `S(z)` 600 Hz 위 재현 불가 (0.74–0.84) | 기능 1 고역 전체 | 상쇄 스피커·장착 개선 | **하드웨어** |
| 3 | I/O 왕복 30.4 ms = 루프의 **85%** | closed-loop 대역폭 | 블록 256 → 128 | 재측정·재학습 |
| 4 | 전원 30W (CPU 79% 클록) | 전 구간 | **`nvpmodel -m 0` + 재부팅** | 재부팅 |
| 5 | `machine` 그룹 8개, val/test 각 1그룹 | G4 판정 신뢰도 | 녹음 보강 | 스피커 |
| 6 | 추론 1.5 ms = 루프의 4.3% | 미미 | (CPU/GPU 선택) | — |

### 5.2 "성능이 남는데 왜 가벼운 모델을 쓰나"

정직한 답은 **"용량이 병목이라는 증거가 아직 없다"**이며, 근거와 반론을 함께 적는다.

**용량을 늘리면 안 된다는 근거 (측정됨)**

| 비교 | 결과 |
|---|---|
| `base` 5.99M vs `tiny` 1.16M | **7종 소스 전부 tiny 우세** |
| 최악 아이템 fullband | base **+13.89 dB 증폭** vs tiny +4.06 — do-no-harm 위반 |
| 구조 탐색 20k (4후보) | 어느 후보도 `tiny`를 못 이김. attention 계열 **실격** |

**용량을 늘려야 한다는 근거 (미검증)**

- 위 비교는 전부 **surrogate 플랜트 + 합성 데이터**에서 나왔다. 실측 물리와 recorded
  데이터로는 **한 번도 비교한 적이 없다.**
- `base`의 실패 양상(최악 아이템 증폭)은 **용량 한계가 아니라 과적합 신호**다. 데이터가
  늘거나 물리가 정확해지면 결론이 뒤집힐 수 있다.
- `tiny_wide`(12.8M, 블록 8)는 GPU에서 P99 4.63 ms로 **실시간에 들어간다.** 지금까지의
  구조 탐색은 깊이만 바꿨고 **폭만 키운 변형은 시험한 적이 없다.**

**판별 기준 — 파인튜닝이 끝나면 결정된다**

| 관측 | 진단 | 다음 수 |
|---|---|---|
| 학습 손실 ↓ 인데 recorded 성능 ↛ | **데이터** 부족 | 계열 보강 (§5.1-5) |
| 학습 손실 자체가 정체 | **용량** 부족 | **`tiny_wide` 사전학습 + 파인튜닝** |
| 둘 다 개선 | 현 경로 유지 | Stage-3 closed-loop |

`tiny_wide` 실험 비용: 사전학습 100k + 파인튜닝 50k ≈ 8–10시간 × ₩2,000 = **약 ₩20,000**.
지연·ONNX·TRT 엔진은 이미 준비돼 있다.

> **대가도 함께 본다.** `tiny_wide`는 예산의 **87%**(P99 4.63/5.33 ms)를 쓴다. 현재
> `tiny`+CPU는 29%다. 여유가 줄면 OS 지터 흡수력이 떨어져 xrun 위험이 오른다. 용량
> 이득이 그 위험을 살 만큼 큰지는 **측정으로** 판단한다.

### 5.3 순서

1. **파인튜닝 완료 → G4 측정** (진행 중, ~1.5시간)
2. G4가 지목하는 병목에 따라 §5.2 판별표대로 분기
3. `nvpmodel -m 0` 재부팅 후 지연 전면 재측정 — 모든 선택의 전제가 바뀐다
