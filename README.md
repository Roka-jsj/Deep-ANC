...
| 엔진 | P50 | P90 | P99 | max |
|---|---:|---:|---:|---:|
| **tiny + ORT CPU** | 1.58 | 1.71 | **1.84** | **2.12** |
| tiny + TRT FP16 | **1.32** | 5.68 | 7.66 | 17.92 |
| base + TRT FP16 | 4.96 | 7.55 | 10.14 | 14.93 |

TRT의 P50은 ORT보다 **빠르다**(1.32 vs 1.58 ms). 무너지는 것은 꼬리다. 커널이 0.55 ms 밖에
안 돼 Jetson GPU 클록이 계속 램프다운하고(`trtexec` 변동계수 53–56%), 스텝마다 H2D/D2H가
붙는다. **실시간 마감에서 의미 있는 것은 평균이 아니라 꼬리**이므로 ORT CPU가 옳은 선택이고,
max 2.12 ms 는 5.33 ms 예산에 충분한 여유를 남긴다.
