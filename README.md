...
| **TensorRT FP16 (tiny / base)** | P99 **7.66 / 10.14ms** — CPU보다 나쁘다 | **기각** |
...
**TensorRT는 이 워크로드에서 기각됐다.** `base`를 배포 가능하게 만들 경로로 열어두고 있었으나
실측이 닫았다.

| 엔진 | 클록 기본 P99 | 클록 고정 P99 | max (고정) |
|---|---:|---:|---:|
| **tiny + ORT CPU** | **1.83** | 2.35 | 2.48 |
| tiny + TRT FP16 | 7.66 | **5.46** | 5.53 |
| base + TRT FP16 | 10.14 | — | — |

TRT의 P50은 ORT보다 **빠르다**(1.11 vs 1.49 ms). 무너지는 것은 꼬리다. 커널이 0.55 ms 밖에
안 돼 GPU 가 시간의 90% 를 놀고, 거버너가 클록을 내리며, 다음 호출이 식은 GPU 를 친다
(`trtexec` 변동계수 53–56%). 여기에 스텝마다 H2D/D2H 왕복이 붙는다.

`jetson_clocks` 로 클록을 최대 고정하면 진단이 맞았음이 확인된다 — GPU P99 가 7.66 → 5.46 ms
로 크게 좋아진다. **그래도 못 넘는다**: 5.46 ms 는 블록 예산 5.33 ms 자체와 같아 매 블록
마감을 놓친다. 그리고 클록 고정은 **CPU 경로를 깎는다**(1.83 → 2.35 ms) — Jetson 은 30 W 를
CPU/GPU 가 나눠 쓰므로 GPU 가 계속 최대 클록을 물면 CPU 여유가 줄어든다.

즉 "GPU 를 최대한 활용" 하면 통과하던 경로까지 나빠진다. 목표는 GPU 활용률이 아니라
**5.33 ms 마감을 한 번도 놓치지 않는 것**이고, 이 워크로드에서 둘은 반대 방향이다.
CPU 경로의 max 2.04 ms 는 예산의 38% 만 쓰며, 그 여유가 OS 스케줄링 지터를 흡수해
xrun 을 20초당 2회로 유지한다. Jetson GPU 를 비워두면 RT 오디오 스레드와 열·전력을
다투지 않는다는 이득도 있다.

> 설치된 `onnxruntime 1.18.1` 은 CPU/Azure EP 만 제공한다(`onnxruntime-gpu` 아님).
> 따라서 GPU 경로는 TensorRT 하나뿐이며, 위 결과로 닫혔다.
